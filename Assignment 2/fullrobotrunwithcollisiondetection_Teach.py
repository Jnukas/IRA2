...
def teach_multi_swift(robots: dict, env, safety, default="CR16", dt=0.02):
    ...
    - robots: dict like {"CR16": cr16, "CR3": cr3}
    - default: which robot is active at start
    """
    # Active robot name holder
...
# Option A: accept any extra args, capture name via keyword-only param
        ...
        for j in range(rbt.n):
            ...
            if pris:
                ...
                def on_change(v, name=name, rbt=rbt, j=j, ui_min=ui_min, ui_max=ui_max):
                    # Gate by E-STOP and active robot
                    ...
                    rbt.q = q.tolist()
                    try:
                        env.step(0)  # Update the visualization
                    except Exception:
                        pass
                    refresh_labels()
    ...
    # Background label refresher (no env.step here; your script already steps env)
...
def main():
    ...
    # ---- Teach panel for CR16 + CR3 ----   <<<< INSERT HERE (outside the CR16 'if' block)
    robots = {}
    if cr16 is not None:
        robots["CR16"] = cr16
    if cr3 is not None:
