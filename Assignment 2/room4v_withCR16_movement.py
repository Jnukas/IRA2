...
# =====================================================================
# Adjust these values to modify pick/place behavior
PICK_PLACE_CONFIG = {
    "APPROACH_OFFSET": 0.15,      # meters above target for approach
    "GRASP_OFFSET": -0.02,        # meters from pot top (negative = inside pot)
    "PLACE_HEIGHT": 0.20,         # meters above stove surface
    "ELBOW_UP": True,             # Use elbow-up configuration
    "MOVEMENT_TIME": 3.0,         # seconds for each trajectory
}
...
def main():
    ...
    # Floor objects
    stove, stove_top_z = add_mesh(
        "STOVE", "Stove.stl",
        rotation_z=math.pi,
        color=[0.70, 0.70, 0.70, 1.0]
    )
    print(f"✓ Stove top surface at Z = {stove_top_z:.4f} m")
    ...
    # Objects on table 1
    pot, pot_top_z = add_mesh(
        "POT", "Potwithoutthelid.stl",
        z_base=table_top_z + SMALL_GAP,
        color=[1.0, 0.0, 0.0, 1.0]
    )
    print(f"✓ Pot top surface at Z = {pot_top_z:.4f} m")
    ...
    print(f"  Pot top = {pot_top_z:.4f} m")
    print(f"  Stove top = {stove_top_z:.4f} m")
    ...
    print("   • Edit PICK_PLACE_CONFIG for pick/place behavior")
    print("="*70 + "\n")

    # -------------------------
    # Pick and Place Helper Functions
    # -------------------------
    def check_reach_and_solve(robot, x, y, z, q0=None, elbow_up=True, desc="target"):
        """
        Check if target is reachable and return IK solution with elbow configuration.
        """
        ...
        try:
            # Use ikine_LM for better convergence
            sol = robot.ikine_LM(T_goal, q0=q0)
            
            if not sol.success:
                print(f"❌ IK failed for {desc} at ({x:.3f}, {y:.3f}, {z:.3f})")
                return False, None
            
            # Check joint limits
            if robot.islimit(sol.q):
                print(f"❌ Joint limits exceeded for {desc} at ({x:.3f}, {y:.3f}, {z:.3f})")
                return False, None
            
            # Apply elbow-up configuration if needed (adjust joint 3 or elbow joint)
            q_solution = sol.q.copy()
            if elbow_up and len(q_solution) >= 3:
                # For typical 6-DOF arms, joint index 2 (3rd joint) is often the elbow
                # Adjust if your robot has different kinematics
                pass
            
            print(f"✓ {desc} at ({x:.3f}, {y:.3f}, {z:.3f}) is reachable")
            return True, q_solution
            
        except Exception as e:
            print(f"❌ IK exception for {desc}: {e}")
            return False, None

    def execute_trajectory(robot, q_start, q_goal, safety_ctrl, desc="motion"):
        """
        Execute a smooth trajectory between two joint configurations.
        """
        T = PICK_PLACE_CONFIG["MOVEMENT_TIME"]
        t = np.arange(0, T + DT, DT)
        traj = rtb.jtraj(q_start, q_goal, t)
        
        print(f"[CR16] Executing {desc}...")
        for qk in traj.q:
            safety_ctrl.block_until_allowed(env, DT)
            robot.q = qk
            env.step(DT)
            time.sleep(DT)

    # -------------------------
    # CR16 Pick and Place Sequence
    # -------------------------
    if RUN_CR16_PICK_PLACE and cr16 is not None:
        print("\n" + "="*70)
        print("[CR16 PICK AND PLACE] Starting sequence")
        print("="*70)
        
        # Calculate pick and place coordinates
        pot_x, pot_y = POSITIONS["POT"]
        stove_x, stove_y = POSITIONS["STOVE"]
        
        # Pick coordinates (at pot top center + grasp offset)
        pick_z = pot_top_z + PICK_PLACE_CONFIG["GRASP_OFFSET"]
        approach_z = pick_z + PICK_PLACE_CONFIG["APPROACH_OFFSET"]
        
        # Place coordinates (above stove)
        place_z = stove_top_z + PICK_PLACE_CONFIG["PLACE_HEIGHT"]
        place_approach_z = place_z + PICK_PLACE_CONFIG["APPROACH_OFFSET"]
        
        print(f"\n📍 Pick location: ({pot_x:.3f}, {pot_y:.3f}, {pick_z:.3f})")
        print(f"📍 Place location: ({stove_x:.3f}, {stove_y:.3f}, {place_z:.3f})")
        print(f"📍 Approach offset: {PICK_PLACE_CONFIG['APPROACH_OFFSET']:.3f} m")
        
        # Check all waypoints for reachability
        q_current = cr16.q.copy()
        elbow_up = PICK_PLACE_CONFIG["ELBOW_UP"]
        
        # 1. Approach above pot
        reach_approach, q_approach = check_reach_and_solve(
            cr16, pot_x, pot_y, approach_z, q_current, elbow_up, "approach above pot"
        )
        
        if not reach_approach:
            print("\n❌ POT IS UNREACHABLE - Approach position cannot be reached by CR16")
            print("   Try adjusting:")
            print("   • POSITIONS['POT'] or POSITIONS['CR16']")
            print("   • PICK_PLACE_CONFIG['APPROACH_OFFSET']")
        else:
            # 2. Pick position
            reach_pick, q_pick = check_reach_and_solve(
                cr16, pot_x, pot_y, pick_z, q_approach, elbow_up, "pick position"
            )
            
            if not reach_pick:
                print("\n❌ POT IS UNREACHABLE - Pick position too low for CR16")
                print("   Try adjusting:")
                print("   • PICK_PLACE_CONFIG['GRASP_OFFSET'] (make less negative)")
                print("   • HEIGHT_OFFSETS['POT'] (raise the pot)")
            else:
                # 3. Lift position (back to approach height)
                reach_lift, q_lift = check_reach_and_solve(
                    cr16, pot_x, pot_y, approach_z, q_pick, elbow_up, "lift position"
                )
                
                # 4. Place approach
                reach_place_app, q_place_approach = check_reach_and_solve(
                    cr16, stove_x, stove_y, place_approach_z, q_lift, elbow_up, "place approach"
                )
                
                if not reach_place_app:
                    print("\n❌ STOVE IS UNREACHABLE - Place approach position cannot be reached")
                    print("   Try adjusting:")
                    print("   • POSITIONS['STOVE']")
                    print("   • PICK_PLACE_CONFIG['PLACE_HEIGHT']")
                else:
                    # 5. Place position
                    reach_place, q_place = check_reach_and_solve(
                        cr16, stove_x, stove_y, place_z, q_place_approach, elbow_up, "place position"
                    )
                    
                    if not reach_place:
                        print("\n❌ PLACE POSITION UNREACHABLE")
                    else:
                        # All waypoints reachable - execute sequence
                        print("\n✅ All waypoints reachable! Executing pick and place...\n")
                        
                        # Move to approach
                        execute_trajectory(cr16, q_current, q_approach, safety, "move to approach")
                        
                        # Move down to pick
                        execute_trajectory(cr16, q_approach, q_pick, safety, "move to pick")
                        
                        print("[CR16] 🤏 Grasping pot...")
                        time.sleep(0.5)
                        
                        # Lift pot
                        execute_trajectory(cr16, q_pick, q_lift, safety, "lift pot")
                        
                        # Move to place approach
                        execute_trajectory(cr16, q_lift, q_place_approach, safety, "move to stove")
                        
                        # Lower to place
                        execute_trajectory(cr16, q_place_approach, q_place, safety, "lower to place")
                        
                        print("[CR16] 📦 Releasing pot...")
                        time.sleep(0.5)
                        
                        # Retract
                        execute_trajectory(cr16, q_place, q_place_approach, safety, "retract")
                        
                        print("\n✅ Pick and place complete!")

    # -------------------------
    # Phase 2: Additional Motions
    # -------------------------
    q = ur3.q.copy()
    q[0] = 0.0
    ur3.q = q
