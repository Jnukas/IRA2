...
=======
    ...
    "CHICKEN": (0.25, -1),
    "UR3": (0, -0.5),
>>>>>>> 36bd9d86e36176f890ff579c4b4ddb2904dbb487:Assignment 2/trial_1.py
    "CR3": (-0.5, -0.4),
    "CR16": (-0.5, -1.5),
    "BARRIER" : (-0.75, 1.25),
}
...
def main():
    ...
    # Objects on tables
    pot, _ = add_mesh("POT", "Potwithoutthelid.stl", z_base=TABLE_HEIGHT + SMALL_GAP, color=[1.0, 0.0, 0.0, 1.0])
    jug, _ = add_mesh("JUG", "jugfixed.stl", z_base=TABLE_HEIGHT + SMALL_GAP, color=[0.8, 0.9, 1.0, 1.0])
    pepper_grinder, _ = add_mesh("PEPPER", "pepper_grinder.stl", z_base=TABLE_HEIGHT + SMALL_GAP, color=[0.2, 0.2, 0.2, 1.0])
    beef, _ = add_mesh("BEEF", "beef.stl", z_base=TABLE_HEIGHT + SMALL_GAP, color=[0.8, 0.3, 0.3, 1.0])
    fruit_veg_tray, _ = add_mesh("FRUIT_VEG", "Fruit_and_Vegetables_Tray.stl", z_base=TABLE_HEIGHT + SMALL_GAP, color=[0.4, 0.7, 0.3, 1.0])
    chicken, _ = add_mesh("CHICKEN", "chicken.stl", z_base=TABLE_HEIGHT + SMALL_GAP, color=[1.0, 0.9, 0.7, 1.0])
