...
def teach_cr16_swift(robot, env, safety, dt=0.02):
    """
    Swift-native teach UI for a robot:
      - Sliders show MECHANICAL angle (q + offset) in degrees for revolute joints,
        metres for prismatic joints.
      - Live pose labels XYZ/ RPY (deg)/ quaternion [w,x,y,z].
      - E-STOP/Resume buttons call your SafetyController.
    """
    # -- helpers --------------------------------------------------------------
    def current_T():
    ...
    def pose_strings():
        ...
        # robust across spatialmath versions
    ...
    def qlim_pair(link):
        ...
        if ql is None: return None, None
        arr = np.asarray(ql).ravel()
        return (float(arr[0]), float(arr[1])) if arr.size >= 2 else (None, None)

    # -- labels ---------------------------------------------------------------
    lx = swift.Label(""); env.add(lx)
    ly = swift.Label(""); env.add(ly)
    lz = swift.Label(""); env.add(lz)
    lr = swift.Label(""); env.add(lr)
    lp = swift.Label(""); env.add(lp)
    lyaw = swift.Label(""); env.add(lyaw)
    lq = swift.Label(""); env.add(lq)
    lestop = swift.Label("E-STOP: DISENGAGED"); env.add(lestop)

    def refresh_labels():
        x_s, y_s, z_s, r_s, p_s, yaw_s, q_s = pose_strings()
        lx.desc, ly.desc, lz.desc = x_s, y_s, z_s
        lr.desc, lp.desc, lyaw.desc = r_s, p_s, yaw_s
        lq.desc = q_s

    # -- E-STOP buttons -------------------------------------------------------
    def on_estop(*_):
        safety.engage_e_stop()
        lestop.desc = "E-STOP: ENGAGED"

    def on_resume(*_):
        safety.disengage_e_stop()
        safety.resume()
        lestop.desc = "E-STOP: DISENGAGED"
    ...
    def add_joint_slider(j):
        ...
        if pris:
            ...
            def on_change(v):
                if not safety.is_running():  # soft stop
                    return
                q = np.asarray(robot.q, float)
                q[j] = float(v)
                if ql0 is not None: q[j] = float(np.clip(q[j], ui_min, ui_max))
                robot.q = q.tolist()
                refresh_labels()

            slider = swift.Slider(cb=on_change, min=ui_min, max=ui_max,
                                  step=0.001, value=value, desc=f"CR16 J{j+1}", unit=" m")
            env.add(slider)
            ...
            slider = swift.Slider(cb=on_change, min=ui_min, max=ui_max,
                                  ...
                                  desc=f"CR16 J{j+1}", unit="°")
            env.add(slider)

    for j in range(robot.n):
        add_joint_slider(j)

    # background refresher so labels stay live even when idle
    def tick():
        while True:
            try:
                env.step(dt)
...
def main():
    ...
    teach_cr16_swift(cr16, env, safety, dt=DT)
